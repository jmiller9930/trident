...
|----|-----------|-------|------------------------------|--------------------|--------|
| W-028 | **100P** | Step 3 Build | **`GET /api/v1/locks/active`** + TTL/`expires_at` + stale release; **`get_settings_dep`** Request-based; extension **`editGuard`** + lock commands + settings; compose **`TRIDENT_LOCK_TTL_SEC`**. **`pytest` 83**. **Commit:** **`565b6ae`**. | **PENDING** program ACCEPT | 2026-04-30 |
