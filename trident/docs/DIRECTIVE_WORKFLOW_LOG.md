...
|----|-----------|-------|------------------------------|--------------------|--------|
| W-024 | **100U** | Step 3 Build | **Web UI:** Vite React SPA; nginx **same-origin /api proxy** to **trident-api** (base-path aware); panels for directives CRUD/workflow, memory project+directive, agents ledger, Git limitation+GIT proofs, Nike events, MCP classify/execute + subsystem router; **`vitest`** + **`pytest` 77** PASS. **No** backend/API expansion. **Commit:** **`6841e67`**. | **PENDING** program ACCEPT | 2026-04-30 |
