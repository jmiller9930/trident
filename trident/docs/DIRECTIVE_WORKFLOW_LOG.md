...
|----|-----------|-------|------------------------------|--------------------|--------|
| W-028 | **100P** | Step 3 Build | **`GET /api/v1/locks/active`** + TTL/`expires_at` + stale release; **`get_settings_dep`** Request-based; extension **`editGuard`** + lock commands + settings. **`pytest` 83**. Proof commit TBD. | **PENDING** program ACCEPT | 2026-04-30 |
