...
|----|-----------|-------|------------------------------|--------------------|--------|
| W-027 | **100P** | Phase 2 — Read + Plan | **Step 1 Read** + **Step 2 Plan:** **`100P`** + **100E** locks + **FIX 001**; gaps (**GET active lock**, **`project_id`/`user_id`** in IDE, git display); proposed **`GET /api/v1/locks/active`** + extension **`lockClient`/`editGuard`**. Log: **`WORKFLOW_LOG.md`** §100P. **Commit:** TBD. | **PENDING** — ACK **`100P_PLAN`** | 2026-04-30 |
