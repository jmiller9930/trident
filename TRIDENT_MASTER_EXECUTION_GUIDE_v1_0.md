...
100A → 100B → 100C → 100D → 100E → 100F → 100G → 100H → 100I → 100J → 100L → 100U
...
| **100H** Agent Execution Layer (backend) | **100G** | **000B**, **000F**, **000K** (and refs in **100H** directive) | Subsystem routing + MCP path proven |
...
100A → 100B → 100C → 100D → 100E → 100F → 100G → 100H → 100I → 100J → 100L → 100U; then 100K → 100P → 100M → 100N.
...
Do **not** start **100K** until **100U** is complete (web control plane) and **000O** is the governing IDE architecture reference.
...
5. Continue **100E → 100F → 100G**; complete **100H → 100I → 100J → 100L → 100U** as needed; schedule **100R** and **FIX 005** before production **external LLM** reliance.  
